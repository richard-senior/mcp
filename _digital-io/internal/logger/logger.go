...
func SetMCPMode(mcpMode bool) {
	if mcpMode {
		// In MCP mode, redirect info logs to stderr to avoid interfering with JSON-RPC on stdout
		defaultLogger.infoLogger.SetOutput(os.Stderr)
	} else {
		// In normal mode, info logs go to stdout
		defaultLogger.infoLogger.SetOutput(os.Stdout)
	}
}
